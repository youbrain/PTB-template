...
from base import (SETTINGS_MAIN, SET_PSWD, SET_LOCATION, texts, keyboards, config)
''' settings '''


def get_settings_keyb(user):
	if user.use_stickers:
		stickers = InlineKeyboardButton(keyboards['settings']['headlines']['stickers']+keyboards['settings']['status'][0],
																							callback_data='set_stickers_0')
	else:
		stickers = InlineKeyboardButton(keyboards['settings']['headlines']['stickers']+keyboards['settings']['status'][1],
																							callback_data='set_stickers_1')
	if user.password:
		...
		lock_time = InlineKeyboardButton(keyboards['settings']['headlines']['lock_time'].replace('<t>', str(user.lock_time)),
																								callback_data='set_locktime')
	else:
		pswd_change = InlineKeyboardButton(keyboards['settings']['headlines']['set_password'], callback_data='pswd_set')
		lock_time = None

	if user.coordinates:
		location = InlineKeyboardButton(keyboards['settings']['headlines']['location']+user.coord_place, callback_data='set_location')
	else:
	...
	if lock_time: # if pswd is activeted
...
def set_sth(update, context):
	...
	# locktime change
		...
		if len(data) == 2:
			d = [[texts['settings']['locktime_mins']+a, 'set_locktime_'+a] for a in config['locktime']]
